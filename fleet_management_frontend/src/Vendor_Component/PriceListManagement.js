...
const PriceListManagement = () => {
  ...
  useEffect(() => {
    ...
    if (vendorID) {
      console.log('Loaded vendorID:', vendorID); // Debugging line to confirm correct vendorID
      fetchPriceLists(vendorID);
      fetchVehicleTypes();
    } else {
  ...
  const fetchPriceLists = async (vendorID) => {
    console.log('Fetching price lists for vendorID:', vendorID); // Debugging line
    try {
  ...
  const fetchVehicleTypes = async () => {
    const vendorID = JSON.parse(localStorage.getItem('vendorData'))?.vendorData?.vendorID;
    try {
      const res = await axios.get(`http://localhost:5000/api/vendor/manage-vehicle-types/${vendorID}`);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const vendorID = JSON.parse(localStorage.getItem('vendorData'))?.vendorID;
    const payload = { ...formData, vendorID };

    try {
      if (editId) {
        await axios.put(`http://localhost:5000/api/vendor/manage-pricelists/${editId}`, payload);
        alert('Price list updated');
      } else {
        await axios.post('http://localhost:5000/api/vendor/manage-pricelists', payload);
        alert('Price list added');
      }
      fetchPriceLists(vendorID); // Refresh price lists after submit
      resetForm();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    const vendorID = JSON.parse(localStorage.getItem('vendorData'))?.vendorID;
    try {
      await axios.delete(`http://localhost:5000/api/vendor/manage-pricelists/${id}`);
      alert('Price list deleted successfully');
      fetchPriceLists(vendorID); // Pass vendorID again to fetch updated data
    } catch (err) {
  ...
  return (
    <div className={styles.priceContainer}>
      ...
      >
        ...
        <a style={{ color: "black", fontSize: "20px" }} href='/vendor-manage-vehicle'>Go Back</a>
      </div>
      <form onSubmit={handleSubmit} className={styles.priceForm}>
        <select id="vehicleType" value={formData.vehicleType} onChange={handleInputChange} required>
          ...
          {vehicleTypes
            .filter((type) => {
              // Show all if editing an existing price list
              if (editId && formData.vehicleType === type) return true;
              return !priceLists.some((pl) => pl.VehicleType === type);
            })
            .map((type, idx) => (
